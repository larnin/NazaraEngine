// Copyright (C) 2017 Jérôme Leclercq
// This file is part of the "Nazara Engine - Graphics module"
// For conditions of distribution and use, see copyright notice in Config.hpp

#pragma once

#ifndef NAZARA_SLICEDSPRITE_HPP
#define NAZARA_SLICEDSPRITE_HPP

#include <Nazara/Prerequisites.hpp>
#include <Nazara/Graphics/InstancedRenderable.hpp>
#include <Nazara/Graphics/Material.hpp>
#include <array>

namespace Nz
{
	class SlicedSprite;

	using SlicedSpriteConstRef = ObjectRef<const SlicedSprite>;
	using SlicedSpriteLibrary = ObjectLibrary<SlicedSprite>;
	using SlicedSpriteRef = ObjectRef<SlicedSprite>;

	class NAZARA_GRAPHICS_API SlicedSprite : public InstancedRenderable
	{
		friend SlicedSpriteLibrary;
		friend class Graphics;

		public:
			inline SlicedSprite();
			inline SlicedSprite(MaterialRef material);
			inline SlicedSprite(Texture* texture);
			SlicedSprite(const SlicedSprite&) = default;
			SlicedSprite(SlicedSprite&&) = delete;
			~SlicedSprite() = default;

			void AddToRenderQueue(AbstractRenderQueue* renderQueue, const InstanceData& instanceData, const Recti& scissorRect) const override;

			std::unique_ptr<InstancedRenderable> Clone() const override;

			inline const Color& GetColor() const;
			inline const Vector3f& GetOrigin() const;
			inline const Vector2f& GetSize() const;
			inline const Rectf& GetTextureCoords() const;
			inline float GetTopSliceMargin() const;
			inline float GetDownSliceMargin() const;
			inline float GetLeftSliceMargin() const;
			inline float GetRightSliceMargin() const;

			inline void SetColor(const Color& color);
			inline void SetDefaultMaterial();
			inline void SetMaterial(MaterialRef material, bool resizeSprite = true);
			bool SetMaterial(String materialName, bool resizeSprite = true);
			inline void SetMaterial(std::size_t skinIndex, MaterialRef material, bool resizeSprite = true);
			bool SetMaterial(std::size_t skinIndex, String materialName, bool resizeSprite = true);
			inline void SetOrigin(const Vector3f& origin);
			inline void SetSize(const Vector2f& size);
			inline void SetSize(float sizeX, float sizeY);
			bool SetTexture(String textureName, bool resizeSprite = true);
			inline void SetTexture(TextureRef texture, bool resizeSprite = true);
			bool SetTexture(std::size_t skinIndex, String textureName, bool resizeSprite = true);
			inline void SetTexture(std::size_t skinIndex, TextureRef texture, bool resizeSprite = true);
			inline void SetTextureCoords(const Rectf& coords);
			inline void SetTextureRect(const Rectui& rect);
			inline void SetSliceMargin(float size);
			inline void SetSliceMargin(unsigned int size);
			inline void SetSliceMargin(float top, float down, float left, float right);
			inline void SetSliceMargin(unsigned int top, unsigned int down, unsigned int left, unsigned int right);

			inline SlicedSprite& operator=(const SlicedSprite& sprite);
			SlicedSprite& operator=(SlicedSprite&& sprite) = delete;

			template<typename... Args> static SlicedSpriteRef New(Args&&... args);

		private:
			inline void InvalidateVertices();
			void MakeBoundingVolume() const override;
			void UpdateData(InstanceData* instanceData) const override;
			void UpdateNbRenderedQuads() const;

			static bool Initialize();
			static void Uninitialize();

			Color m_color;
			Rectf m_textureCoords;

			float m_topMargin;
			float m_downMargin;
			float m_leftMargin;
			float m_rightMargin;

			Vector2f m_size;
			Vector3f m_origin;

			mutable unsigned int m_nbQuads;

			static SlicedSpriteLibrary::LibraryMap s_library;
	};
}

#include <Nazara/Graphics/SlicedSprite.inl>

#endif // NAZARA_SLICEDSPRITE_HPP
